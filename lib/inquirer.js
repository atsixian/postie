...
const askPostInfo = async questionFile => {
  const { config } = require("./config");
  templateFile = questionFile || config.get("defaultTemplateFile");
  // no config yet
  let questions;
  try {
    questions = require(`${__dirname}/templates/${templateFile}`);
  } catch (err) {
    console.log(`Please make sure the template "${templateFile}" exists!`);
    process.exit(1);
  }
